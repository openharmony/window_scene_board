...
import { DragContextContent } from '../../common/recover/DragContext';
 ...
 */
export class DragManager implements DragContextContent {
   ...
   * 创建拖拽管理实例
   *
   * @param params 拖拽管理创建参数
   * @returns 拖拽管理实例
   */
  public static build(params: DragParams): DragManager | undefined {
    if (!params) {
      log.showError(`no drag parameters`);
      return undefined;
    }
