...
const TAG = 'DropdownViewControllerAdapter';
const log = LogHelper.getLogHelper(LogDomain.SYS_UI, TAG);

export class DropdownViewControllerAdapter {
  private static get = SingletonHelper.createFactory(() => new DropdownViewControllerAdapter());
  public static get instance(): DropdownViewControllerAdapter {
    return this.get();
  }

  private controller: BaseViewController = new BaseViewController(ViewType.DROPDOWN);
  ...
  @threadCall()
  register(): void {
    log.showInfo('Register dropdown controller');
    viewMgrPolicy.registerViewController(ViewType.DROPDOWN, this.controller);
  }

  @threadCall()
  unregister(): void {
    ViewManagerAdapter.unregisterViewController(ViewType.DROPDOWN);
  }
